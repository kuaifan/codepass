...
# 准备工作
STATUS "Preparing"
mkdir -p {{.WORK_PATH}}/workspaces/{{.NAME}}/config/code-server
mkdir -p {{.WORK_PATH}}/workspaces/{{.NAME}}/workspace
cat > {{.WORK_PATH}}/workspaces/{{.NAME}}/config/info.yaml <<-EOF
...
EOF
cat > {{.WORK_PATH}}/workspaces/{{.NAME}}/config/code-server/config.yaml <<-EOF
app-name: CodePass
...
start="$start --cloud-init {{.WORK_PATH}}/workspaces/{{.NAME}}/config/init.yaml"
start="$start --mount {{.WORK_PATH}}/workspaces/{{.NAME}}/config:~/.config"
start="$start --mount {{.WORK_PATH}}/workspaces/{{.NAME}}/workspace:~/workspace"
